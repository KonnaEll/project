...
int main(int argc, char* argv[])
{
    ...
    for (int i=0;i<input_items_counter; i++){
        for (int j=0;j<k;j++){                      //create 2 dimensional array with random numbers(0 or 1)
            f[i][j]=rand() % 2;
        }
    }
    ...
    int * a=h_p(f,k,input_items_counter); //create table with the results of f(h) for input items
    // Hash table for input file
    int TableSize = pow(2,k);           // calculate table size 
    int hash_index;
    ...
    // we put hash nodes in hash table
    for(int i=0; i<input_items_counter; i++)
    {
        hash_index =a[i];
        ID=hash_index;
    ...
    for (i=0;i<query_items_counter; i++){
        for (j=0;j<k;j++){
            f1[i][j]=rand() % 2;        //create 2 dimensional array with random numbers(0 or 1)
        }
    }
    ...
    int * a1=h_p(f1,k,query_items_counter);  //create table with the results of f(h) for queries
       ...
       // fprintf(output_file_ptr, "Hash Table no%d\n", g);
        ...
        for(int m=0; m<query_items_counter; m++)    // for every query show the results
        {
                ...
                {       
                    ...
                    // printf("hdi=%d\n",hdi);
                         if((k_ID == hash_tables[hash_index]->ID || hdi<=probes ) && cp<=M)  // compare the IDs or haming distance <=probes and counter of points<=M
                        {
                            ...
                            dist = sqrt(dist);
                            if(dist < min_dist) // minimun cube distance
                            {
                                min_dist = dist;
            ...
            free(k_min_dist);
            free(k_nearest_neighbor);    //allocate memory
            free(radius);
